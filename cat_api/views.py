...
CAT_API_HEADERS = {"x-api-key": CAT_API_KEY, "Content-Type": "application/json"}
    ...
    def sync(self, request):
        ...
        try:
            response = requests.get(
                f"{CAT_API_BASE_URL}/breeds", headers=CAT_API_HEADERS
            )
            ...
            for breed_data in breeds_data:
                breed, created = CatBreed.objects.update_or_create(
                    ...
                    defaults={
                        ...
                        "weight_imperial": breed_data.get("weight", {}).get(
                            "imperial", ""
                        ),
                    ...
                    for breed_data in image_data["breeds"]:
                        breed, _ = CatBreed.objects.get_or_create(
                            ...
                            defaults={
                                ...
                                "weight_metric": breed_data.get("weight", {}).get(
                                    "metric", ""
                                ),
                            },
                        )
...
class CatFavoriteViewSet(viewsets.ModelViewSet):
    ...
    def remove(self, request):
        ...
        try:
            ...
            favorite.delete()
            return Response(
                {"success": True, "message": "Image removed from favorites"}
            )
...
def cat_breed_detail(request, breed_id):
    ...
    return render(
        request, "cat_api/breed_detail.html", {"breed": breed, "images": images}
    )
...
def cat_demo(request):
    ...
    if request.user.is_authenticated:
        favorites = CatFavorite.objects.filter(user=request.user).order_by(
            "-created_at"
        )[:8]

    context = {"breeds": breeds, "favorites": favorites}
...
def random_cats(request):
    ...
    if request.user.is_authenticated:
        favorites = CatFavorite.objects.filter(
            user=request.user, image__in=images
        ).values_list("image_id", flat=True)

    context = {"images": images, "favorites": favorites}
...
def breed_images(request):
    ...
    try:
        ...
        if images.count() < limit:
            try:
                response = requests.get(
                    f"{CAT_API_BASE_URL}/images/search",
                    headers=CAT_API_HEADERS,
                    params={"breed_ids": breed_ids, "limit": limit},
                )
                ...
                if response.status_code == 200:
                    ...
                    for image_data in images_data:
                        # Check if image already exists
                        if not CatImage.objects.filter(
                            image_id=image_data["id"]
                        ).exists():
                            # Create new image
                            image = CatImage.objects.create(
        ...
        context = {"images": images, "breed": breed, "favorites": favorites}

        return render(request, "cat_api/partials/breed_images.html", context)

    except CatBreed.DoesNotExist:
        return JsonResponse(
            {"error": f"Breed with ID {breed_ids} not found"}, status=404
        )
...
def breed_detail_api(request, breed_id):
    ...
    try:
        ...
        return JsonResponse(
            {"error": f"Breed with ID {breed_id} not found"}, status=404
        )
...
def add_favorite(request):
    ...
    try:
        ...
        return render(
            request,
            "cat_api/partials/favorite_button.html",
            {"image": image, "is_favorite": True},
        )

    except CatImage.DoesNotExist:
        return JsonResponse(
            {"error": f"Image with ID {image_id} not found"}, status=404
        )
