...
class Command(BaseCommand):
    ...
    def load_breeds(self):
        ...
        if response.status_code != 200:
            self.stdout.write(self.style.ERROR(f'Failed to fetch breeds: {response.status_code}'))
            return
    ...
    def load_breed_images(self, images_per_breed):
        ...
        for breed in breeds:
            ...
            if response.status_code != 200:
                self.stdout.write(self.style.ERROR(f'Failed to fetch images for {breed.name}: {response.status_code}'))
                continue
    ...
    def load_random_images(self, count):
        ...
        if response.status_code != 200:
            self.stdout.write(self.style.ERROR(f'Failed to fetch random images: {response.status_code}'))
            return
