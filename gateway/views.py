...
from .serializers import (
    ...
    RequestTransformationSerializer,
    ResponseTransformationSerializer,
    ApiLogSerializer,
)
...
class ApiEndpointViewSet(viewsets.ModelViewSet):
    ...
    def logs(self, request, pk=None):
        ...
        endpoint = self.get_object()
        logs = endpoint.logs.all().order_by("-created_at")[
            :100
...
class RequestTransformationViewSet(viewsets.ModelViewSet):
    ...
    queryset = RequestTransformation.objects.all().order_by(
        "endpoint__domain__name", "endpoint__path"
    )
    serializer_class = RequestTransformationSerializer
    filter_backends = [filters.SearchFilter, filters.OrderingFilter]
    search_fields = [
        "endpoint__domain__name",
        "endpoint__path",
        "source_field",
        "target_field",
    ]
...
class ResponseTransformationViewSet(viewsets.ModelViewSet):
    ...
    queryset = ResponseTransformation.objects.all().order_by(
        "endpoint__domain__name", "endpoint__path"
    )
...
def dashboard(request):
    ...
    transformations_count = (
        RequestTransformation.objects.filter(is_active=True).count()
        + ResponseTransformation.objects.filter(is_active=True).count()
    )
        ...
        for i in range(len(req_source_fields)):
            if req_source_fields[i] and req_target_fields[i]:
                RequestTransformation.objects.create(
                    ...
                    transformation_value=req_transform_values[i]
                    if i < len(req_transform_values)
                    else "",
                    is_active=True,
                )
        ...
        for i in range(len(res_source_fields)):
            if res_source_fields[i] and res_target_fields[i]:
                ResponseTransformation.objects.create(
                    ...
                    transformation_value=res_transform_values[i]
                    if i < len(res_transform_values)
                    else "",
                    is_active=True,
                )
...
def edit_rule(request, endpoint_id):
    ...
    if request.method == "POST":
        ...
        RequestTransformation.objects.filter(endpoint=endpoint).exclude(
            id__in=existing_req_ids
        ).delete()
        ...
        for i in range(len(req_source_fields)):
            if req_source_fields[i] and req_target_fields[i]:
                transform_id = (
                    req_transform_ids[i]
                    if i < len(req_transform_ids) and req_transform_ids[i]
                    else None
                )
                ...
                if transform_id:
                    ...
                    transform = get_object_or_404(
                        RequestTransformation, id=transform_id
                    )
                    ...
                    transform.transformation_value = (
                        req_transform_values[i] if i < len(req_transform_values) else ""
                    )
                    transform.save()
                else:
        ...
        ResponseTransformation.objects.filter(endpoint=endpoint).exclude(
            id__in=existing_res_ids
        ).delete()
        ...
        for i in range(len(res_source_fields)):
            if res_source_fields[i] and res_target_fields[i]:
                transform_id = (
                    res_transform_ids[i]
                    if i < len(res_transform_ids) and res_transform_ids[i]
                    else None
                )
                ...
                if transform_id:
                    # Update existing transformation
                    transform = get_object_or_404(
                        ResponseTransformation, id=transform_id
                    )
                    ...
                    transform.transformation_value = (
                        res_transform_values[i] if i < len(res_transform_values) else ""
                    )
                    transform.save()
                else:
                    # Create new transformation
...
def create_domain_ajax(request):
    if request.method == "POST":
        ...
        if Domain.objects.filter(name=name).exists():
            return JsonResponse(
                {"success": False, "error": f'Domain "{name}" already exists'}
            )
        ...
        return JsonResponse(
            {"success": True, "domain": {"id": domain.id, "name": domain.name}}
        )
...
def log_detail_ajax(request):
    ...
    if not log_id:
        return JsonResponse({"success": False, "error": "Log ID is required"})
