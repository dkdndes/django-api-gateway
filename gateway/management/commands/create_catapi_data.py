...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if created:
            self.stdout.write(self.style.SUCCESS(f'Created endpoint: {images_search_endpoint}'))
        else:
        ...
        if created:
            self.stdout.write(self.style.SUCCESS(f'Created endpoint: {images_search_apikey_endpoint}'))
        else:
            self.stdout.write(f'Endpoint already exists: {images_search_apikey_endpoint}')
        ...
        if created:
            self.stdout.write(self.style.SUCCESS(f'Created endpoint: {image_by_id_endpoint}'))
        else:
        ...
        if created:
            self.stdout.write(self.style.SUCCESS(f'Created response transformation: {first_image_transform}'))
        else:
            self.stdout.write(f'Response transformation already exists: {first_image_transform}')
        
        # Response transformation to create a simplified response
        simplified_response_transform, created = ResponseTransformation.objects.get_or_create(
            endpoint=images_search_endpoint,
            source_field='0',
        ...
        if created:
            self.stdout.write(self.style.SUCCESS(f'Created response transformation: {simplified_response_transform}'))
        else:
            self.stdout.write(f'Response transformation already exists: {simplified_response_transform}')
