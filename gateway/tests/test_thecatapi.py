...
def test_cat_api_request_success(mock_get, middleware, request_factory, cat_api_endpoint):
    """Test successful Cat API request handling"""
    # Mock the response from the Cat API
    mock_response = MagicMock()
    mock_response.status_code = 200
    mock_response.headers = {"Content-Type": "application/json"}
    mock_response.json.return_value = [{"id": "33j", "url": "https://cdn2.thecatapi.com/images/33j.gif", "width": 499, "height": 316}]
    mock_response.text = json.dumps([{"id": "33j", "url": "https://cdn2.thecatapi.com/images/33j.gif", "width": 499, "height": 316}])
    ...
    assert json.loads(response.content) == [{"id": "33j", "url": "https://cdn2.thecatapi.com/images/33j.gif", "width": 499, "height": 316}]
...
@pytest.mark.django_db
@patch("gateway.middleware.requests.get")
def test_cat_api_request_with_apikey_in_url(mock_get, middleware, request_factory, cat_api_endpoint_with_apikey):
    ...
    mock_get.assert_called_once()
    args, kwargs = mock_get.call_args
    assert args[0] == "https://api.thecatapi.com/v1/images/search?api_key=ce4e6963-68ed-4df6-9d5a-40fee969ff84"


@pytest.mark.django_db
@patch("gateway.middleware.requests.get")
def test_cat_api_request_with_query_params(mock_get, middleware, request_factory, cat_api_endpoint):
...
@pytest.mark.django_db
@patch("gateway.middleware.requests.get")
def test_cat_api_request_with_apikey_header(mock_get, middleware, request_factory, cat_api_endpoint):
    ...
    args, kwargs = mock_get.call_args
    assert args[0] == "https://api.thecatapi.com/v1/images/search"
    assert kwargs.get("headers", {}).get("X-Api-Key") == "ce4e6963-68ed-4df6-9d5a-40fee969ff84"