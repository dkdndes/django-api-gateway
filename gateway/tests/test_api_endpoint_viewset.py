...
class ApiEndpointViewSetTestCase(TestCase):
    ...
    def test_request_transformations_action(self):
        ...
        url = reverse(
            "gateway:apiendpoint-request-transformations", args=[self.endpoint.id]
        )
        response = self.client.get(url)
        ...
        self.assertEqual(
            response.data[0]["source_field"], self.req_transform1.source_field
        )
        self.assertEqual(
            response.data[0]["target_field"], self.req_transform1.target_field
        )
        self.assertEqual(
            response.data[1]["source_field"], self.req_transform2.source_field
        )
        self.assertEqual(
            response.data[1]["target_field"], self.req_transform2.target_field
        )

    def test_response_transformations_action(self):
        """Test the custom 'response_transformations' action on the ApiEndpointViewSet."""
        url = reverse(
            "gateway:apiendpoint-response-transformations", args=[self.endpoint.id]
        )
        response = self.client.get(url)
        ...
        # Verify specific transformation data
        self.assertEqual(
            response.data[0]["source_field"], self.res_transform1.source_field
        )
        self.assertEqual(
            response.data[0]["target_field"], self.res_transform1.target_field
        )
        self.assertEqual(
            response.data[1]["source_field"], self.res_transform2.source_field
        )
        self.assertEqual(
            response.data[1]["target_field"], self.res_transform2.target_field
        )
