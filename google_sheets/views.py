...
class GoogleSheetViewSet(viewsets.ModelViewSet):
    ...
    def sync(self, request, pk=None):
        ...
        try:
            ...
            if not values:
                return Response({'message': 'No data found in sheet'}, status=status.HTTP_404_NOT_FOUND)
...
class SheetDataViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    def get_queryset(self):
        ...
        if sheet_id:
            return SheetData.objects.filter(sheet__id=sheet_id, sheet__user=self.request.user)
